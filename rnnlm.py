...
class RNNLM(NNBase):
    ...
    def __init__(self, L0, U0=None,
        ...
        if U0 is None:
            self.params.U = sigma * np.random.randn(param_dims["U"]) + mu
        else:
    ...
    def _acc_grads(self, xs, ys):
        ...
        # Forward propagation
        for t in xrange(ns):
            hs[t + 1] = sigmoid(H.dot(hs[t]) + L[xs[t]])
        ...
        for t in xrange(ns):
            ...
            delta_t = ps[t] - y_t
            #self.grads.U += delta_t.dot(hs[t])
            self.grads.U += outer(delta_t, hs[t])
            delta = U.T.dot(delta_t) * hs[t] * (1 - hs[t])
            for i in xrange(t, t - bptt - 1, -1):
                if i - 1 >= 0:
                    self.grads.H += outer(delta, hs[i - 1])
                    self.sgrads.L[xs[i]] = delta
                    delta = H.T.dot(delta) * hs[i - 1] * (1 - hs[i - 1])
